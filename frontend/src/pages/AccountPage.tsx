...
export default function AccountPage() {
  ...
  const handleCreateQR = useCallback(async () => {
    try {
      ...
      if (res.ok && res.uuid) {
        ...
        setQrImage(
          img ||
            `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(res.scan_url || '')}`,
        );
        setQrVisible(true);
          ...
          <Space size="small">
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  await api.toggleAccount(record.id, ns);
                  message.success(isActive ? 'disabled' : 'enabled');
                  loadStatus();
                } catch {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      ...
      {/* QR Code Modal */}
      <Modal title="扫码登录" open={qrVisible} onCancel={handleCloseQR} footer={null} width={400}>
        <div style={{ textAlign: 'center' }}>
          ...
          <Alert type="info" message="打开微信 → 扫一扫 → 确认登录" style={{ marginTop: 12 }} />
        </div>
      </Modal>
    </div>
  );
