    ...
    def validate(
        ...
        *,
        ...
        for check in self.rule_checks:
            ...
            if err:
                return GoalValidationResult(
                    status="incomplete",
                    rules_ok=False,
                    reason=f"rule check failed: {err}",
                    validated_at=stamp,
                )
        ...
        return GoalValidationResult(
            ...
            failed_steps=failed,
            budget_risk=budget_risk,
            reason="not all criteria satisfied",
            validated_at=stamp,
        )
...
class LoopDetector:
    ...
    def detect_loop(self, state: RuntimeState) -> LoopSignal:
        ...
        if (
            len(last_n) >= self.no_evidence_limit
            and all(d.outcome != "success" for d in last_n)
            and not state.evidence
        ):
            return LoopSignal(
                detected=True, kind="no_new_evidence", detail="no new evidence in recent steps"
            )
    ...
    *,
    ...
    if goal_result is not None and goal_result.status == "complete":
        return TerminationDecision(
            ...
            reason="goal validated complete",
    ...
    if broken:
        return TerminationDecision(
            ...
            reason=f"budget exceeded: {broken}",
    ...
    if loop_signal is not None and loop_signal.detected:
        return TerminationDecision(
            stop=True,
            status=RuntimeStatus.STOPPED,
            reason=f"loop detected: {loop_signal.detail}",
